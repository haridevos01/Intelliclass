...
class LoginForm(forms.Form):
    id = forms.CharField(label='ID', max_length=10, validators=[
                         ...
                         widget=forms.TextInput(attrs={'placeholder': 'Enter your ID here'})
                        )
...
def archive(request):
    materials = Material.objects.all().order_by('-datetime')
    for material in materials: 
        material.description = material.description.replace("<p>", "").replace("</p>", "")
    # Pass the materials data to the template
    return render(request, 'main/archive.html', {'materials': materials})